...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let playSoundAction = SKAction.playSoundFileNamed("sound.mp3", waitForCompletion: false)
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            ...
            item.zPosition = -80.0 // 雲より手前、壁より奥
            ...
            sprite.position = CGPoint(x: 0.0, y: item_y)
        ...
        let waitAnimation = SKAction.wait(forDuration: 2)
        // アイテムを作成->待ち時間->アイテムを作成　を無限に繰り替えるアクションを作成
        let repeatForeverAnimation = SKAction.repeatForever(SKAction.sequence([createItemAnimation, waitAnimation]))
        //　１秒待ってから、アイテム作成
        itemNode.run(SKAction.sequence([SKAction.wait(forDuration: 1), repeatForeverAnimation]))
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            // アイテムを取り除く
            let itemBody = contact.bodyA.categoryBitMask > contact.bodyB.categoryBitMask ? contact.bodyA : contact.bodyB
